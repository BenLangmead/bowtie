...
/**
 * An aligner for finding paired alignments by jumping back and forth
 * between Burrows-Wheeler and reference domains.
 *
 *
 */
template<typename TRangeSource, typename TContMan>
class Paired2DomainAlignerV1 : public Aligner {
...
public:
	Paired2DomainAlignerV1(
		...
		uint32_t maxInsert,
		uint32_t symCeiling,
		std::vector<seqan::String<Dna5> >& os,
		bool rangeMode,
		bool verbose,
		uint32_t seed) :
		Aligner(rangeMode, seed),
	...
	virtual ~Paired2DomainAlignerV1() {
		delete driver1Fw_; driver1Fw_ = NULL;
		delete driver1Rc_; driver1Rc_ = NULL;
		delete driver2Fw_; driver2Fw_ = NULL;
		delete driver2Rc_; driver2Rc_ = NULL;
...
protected:
	            ...
	            bool pairFw)    // whether the pair is being mapped to fw strand
	{
		...
		ret = params_->reportHit(
				...
				rR.stratum,                   // alignment stratum
				oms);                         // # other hits
		assert(ret); // TODO: lift this restriction
		return ret;
	}
		                 ...
		                 bool verbose = false)
	{
		...
		if(offsDstSz > 0) {
			...
			// Find it by scanning
			assert_gt(os_.size(), 0);
			// Now we want to scan the appropriate section of the
			// reference string to find the mate

			// What factors determine the part of the reference we want
			// to look at?
			// 1. The reference location of the alignment being added
			// 2. Whether the alignment being added is on the left or
			//    on the right w/r/t the forward reference strand
		}
		// Push new reference locus and range onto the appropriate
		// parallel mate buffers
		if(newFromL) { offsL.push_back(h); rangesL.push_back(drL.range()); }
