...
#ifdef BOWTIE_MM
#define EBWT_FILE_CLOSE close
#define EBWT_READ_RET ssize_t
#define EBWT_READ read
#define EBWT_FILE int
#define EBWT_FILE_INIT -1
#else
#define EBWT_FILE_CLOSE fclose
#define EBWT_READ_RET size_t
#define EBWT_READ(file, dest, sz) fread(dest, 1, sz, file)
#define EBWT_FILE FILE*
#define EBWT_FILE_INIT NULL
#endif
...
#define Ebwt_STAT_INITS
	...
	~Ebwt() {
		...
		EBWT_FILE_CLOSE(_in1);
		EBWT_FILE_CLOSE(_in2);
#ifdef EBWT_STATS
	...
	EBWT_FILE  _in1;    // input fd for primary index file
	EBWT_FILE  _in2;    // input fd for secondary index file
...
void Ebwt<TStr>::readIntoMemory(bool justHeader, EbwtParams *params) {
	...
	if(_useMm) {
		...
		try {
			...
			if(switchEndian) {
				...
				EBWT_READ_RET r = EBWT_READ(_in1, (void*)this->_plen, this->_nPat*4);
				if(r != (EBWT_READ_RET)(this->_nPat*4)) {
	...
	if(_useMm) {
		...
		if(switchEndian) {
			...
			EBWT_READ_RET r = EBWT_READ(_in1, (void *)this->_rstarts, this->_nFrag*4*3);
			if(r != (EBWT_READ_RET)(this->_nFrag*4*3)) {
	...
	if(_useMm) {
		...
		EBWT_READ_RET r = EBWT_READ(_in1, (void *)this->_ebwt, eh->_ebwtTotLen);
		if(r != (EBWT_READ_RET)eh->_ebwtTotLen) {
	...
	try {
		...
		if(_useMm) {
			...
			if(switchEndian) {
				...
				EBWT_READ_RET r = EBWT_READ(_in1, (void *)this->_ftab, eh->_ftabLen*4);
				if(r != (EBWT_READ_RET)(eh->_ftabLen*4)) {
		...
		if(_useMm) {
			...
			if(switchEndian) {
				...
				EBWT_READ_RET r = EBWT_READ(_in1, (void *)this->_eftab, eh->_eftabLen*4);
				if(r != (EBWT_READ_RET)(eh->_eftabLen*4)) {
	...
	while(true) {
		char c = '\0';
		if(EBWT_READ(_in1, (void *)(&c), (size_t)1) != (EBWT_READ_RET)1) break;
		bytesRead++;
		if(c == '\0') break;
		else if(c == '\n') {
	...
	if(_overrideOffRate < 32) {
		...
		if(switchEndian || offRateDiff > 0) {
			...
			for(uint32_t i = 0; i < offsLen; i += blockMaxSzU32) {
				...
				EBWT_READ_RET r = EBWT_READ(_in2, (void *)buf, block << 2);
				if(r != (EBWT_READ_RET)(block << 2)) {
			...
			if(_useMm) {
				...
				if((offsLen & 0xc0000000) != 0) {
					...
					for(int i = 0; i < 4; i++) {
						EBWT_READ_RET r = EBWT_READ(_in2, (void*)offs, offsLen);
						if(r != (EBWT_READ_RET)(offsLen)) {
					...
					EBWT_READ_RET r = EBWT_READ(_in2, (void*)this->_offs, offsLen << 2);
					if(r != (EBWT_READ_RET)(offsLen << 2)) {
	...
	if(switchEndian || isaRateDiff > 0) {
		...
		for(uint32_t i = 0; i < isaLen; i++) {
			if((i & ~(0xffffffff << isaRateDiff)) != 0) {
				char tmp[4];
				EBWT_READ_RET r = EBWT_READ(_in2, (void *)tmp, 4);
				if(r != (EBWT_READ_RET)4) {
		...
		if(_useMm) {
			...
			EBWT_READ_RET r = EBWT_READ(_in2, (void *)this->_isa, isaLen*4);
			if(r != (EBWT_READ_RET)(isaLen*4)) {
