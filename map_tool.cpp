 ...
 * cat MANUAL  | head -650 | tail -340 | sed -e 's/\"/\\\"/g' | \
 *   sed -e 's/^/"/' | sed -e 's/$/\\n"/'
 */
static void printLongUsage(ostream& out) {
	...
	" delimited human-readable format (Bowtie's default output).  (For\n"
	" information about converting Bowtie alignment to Maq's .map format,\n"
	" see the next section on using 'bowtie-maqconvert'.)\n"
	" \n"
	"  Command Line\n"
	"  ------------\n"
	" \n"
