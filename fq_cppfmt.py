...
def parse_args():
    ...
    parser.add_option("-l", "--length",
                      action="store", type="int", dest="max_length",
                      help="(Default: None). Trim sequences to this length.")
    ...
    with open(options.fastq_file, "r") as fin:
        ...
        for rec in fq:
            if num_seqs > options.seqs_count:
                break
            num_seqs += 1
            seq = rec.seq[:options.max_length] if options.max_length is not None else rec.seq
            qual = rec.qual[:options.max_length] if options.max_length is not None else rec.qual
            print "  {"
            print "   \"" + rec.seq_id + "\","
            print "   \"" + seq + "\","
            print "   \"" + qual.replace('\\', '\\\\').replace('"', '\\"').replace('?', '\\?') + "\""
            print "  },"
