...
ebwt_search: ebwt_search.cpp $(OTHER_CPPS) $(HEADERS)
	cat $^ | cksum > .ebwt_search.md5
	m=`cat .ebwt_search.md5 | awk '{print $$1}'` && \
	$(CXX) $(RELEASE_FLAGS) -DEBWT_SEARCH_HASH=$$m -DEBWT_SEARCH_MAIN $(NOASSERT_FLAGS) -Wall $(INC) $(LIBS) -o $@ $< $(OTHER_CPPS)

ebwt_search_prof: ebwt_search.cpp $(OTHER_CPPS) $(HEADERS)
	cat $^ | cksum | sed 's/[01-9][01-9] .*//' > .ebwt_search.md5
	m=`cat .ebwt_search.md5` && \
	$(CXX) $(RELEASE_FLAGS) -DEBWT_SEARCH_HASH=$$m -DEBWT_SEARCH_MAIN $(NOASSERT_FLAGS) -g -pg -Wall $(INC) $(LIBS) -o $@ $< $(OTHER_CPPS)

# Just like ebwt_search_prof but with inlines turned off so that it's
# easier to interpret the gprof results
ebwt_search_prof0: ebwt_search.cpp $(OTHER_CPPS) $(HEADERS)
	cat $^ | cksum | sed 's/[01-9][01-9] .*//' > .ebwt_search.md5
	m=`cat .ebwt_search.md5` && \
	$(CXX) $(RELEASE_FLAGS) -fno-inline -DEBWT_SEARCH_HASH=$$m -DEBWT_SEARCH_MAIN $(NOASSERT_FLAGS) -g -pg -Wall $(INC) $(LIBS) -o $@ $< $(OTHER_CPPS)

ebwt_search-with-asserts: ebwt_search.cpp $(OTHER_CPPS) $(HEADERS)
	cat $^ | cksum | sed 's/[01-9][01-9] .*//' > .ebwt_search.md5
	m=`cat .ebwt_search.md5` && \
	$(CXX) $(DEBUG_FLAGS) -DEBWT_SEARCH_HASH=$$m -DEBWT_SEARCH_MAIN -Wall $(INC) $(LIBS) -o $@ $< $(OTHER_CPPS)

ebwt_search-debug: ebwt_search.cpp $(OTHER_CPPS) $(HEADERS)
	cat $^ | cksum | sed 's/[01-9][01-9] .*//' > .ebwt_search.md5
	m=`cat .ebwt_search.md5` && \
	$(CXX) $(DEBUG_FLAGS) -DEBWT_SEARCH_HASH=$$m -DEBWT_SEARCH_MAIN $(NOASSERT_FLAGS) -Wall $(INC) $(LIBS) -o $@ $< $(OTHER_CPPS)
...
pack_fasta: pack_fasta.cpp packed_io.h endian.cpp tokenize.h tokenize.cpp
	$(CXX) $(DEBUG_FLAGS) -DPACK_FASTA_MAIN -Wall $(INC) $(LIBS) -o $@ $< endian.cpp tokenize.cpp
